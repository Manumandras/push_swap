...
/*   Updated: 2025/08/04 16:41:49 by mmittelb         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	main(int argc, char **argv)
{
	...
	argc = count_nodes(&head_a) + 1;
	if (argc != 1)
	{
		putposition(&head_a);
		if (sorted(&head_a) == 0)
		{
			if (count_nodes(&head_a) < 6)
				tiny_sort(&head_a, &tail_a, count_nodes(&head_a));
			else
				sort(&head_a, &tail_a, argc);
		}
	}
	// while (head_a != NULL)
	// {
